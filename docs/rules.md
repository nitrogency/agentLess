...
# -S all - monitors ALL syscalls made towards specified directory
...
# -F auid!=unset - system daemons have an unset auid. this filters out system daemon calls to this file, which is usually safe. daemons started by the user (after user login) inherit their auid, so these syscalls are still logged.
